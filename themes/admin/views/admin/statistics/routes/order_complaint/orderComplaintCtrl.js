controllers.orderComplaintCtrl = function($scope, $rootScope, $http, $q, commonFactory) {
    ...
    $scope.fetchChartData = function(cb) {
        var url = $scope.local.current_yaxis == 0 ? $request_urls.orderComplaintByDate : $request_urls.orderReturnByDate;
        var query = {
            sort         : {},
            paging       : {
                start : 0,
    ...
    $scope.processChartData = function() {
        ...
        for(i = 0, len = dataset.length; i < len; i++) {
            $scope.data[0].values.push([dataset[i].complaint_date, parseInt(dataset[i].sub_complaint, 10)]);
            $scope.data[1].values.push([dataset[i].return_date, parseInt(dataset[i]['sub_return'], 10)]);
        }
    ...
    $scope.renderChart = function() {
        ...
        nv.addGraph(function() {
            var is_complaint = $scope.local.current_yaxis == '1';
            ...
            var data = [( is_complaint ? $scope.data[1] : $scope.data[0] )];
...
app.controller('orderComplaintCtrl', ['$scope', '$rootScope', '$http', '$q', 'commonFactory', controllers.orderComplaintCtrl]);