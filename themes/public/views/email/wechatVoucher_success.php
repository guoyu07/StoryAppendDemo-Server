...
//$CURRENT_TEMPLATE_URL = 'http://hicart.host/catalog/view/theme/hitour/static/image/common/';
//$CURRENT_TEMPLATE_URL = 'http://192.168.1.126:8000/catalog/view/theme/hitour/static/image/common/';
/*$data = array(
  'product' => '悉尼通票',
  'value' => 100,
  'email' => 'abc@abc.me',
        ...
        <tbody>
          <tr style='height: 338px; background-color: #EAAB13; width: 100%;'>
            ...
            <table style='width: 520px; height: 225px; margin: 0 auto; padding: 55px 0;'>
              ...
              <tr>
                <td style='color: #FFEDC2; height: 30px; line-height: 30px; font-size: 28px; margin-top: 65px;'>恭喜您抢到“<?= $data['product_name']; ?>”<br><?= $data['city_name']; ?>，走起！！</td>
              </tr>
            </table>
            </td>
          </tr>
          <tr>
            <td>
              <table style='width: 520px; margin: 0 auto;'>
                ...
                <tr>
                  <td style='<?= $styleDesc; ?>'>
										该优惠券号码有效期截止到<?= $data['avail_date']; ?><br/><br/>
										1. 通过以下链接，购买“<?= $data['product_name']; ?>”通票（原价<?= sprintf('%.0f', (int)$data['discount'] + (int)$data['special_price']); ?>，输入优惠券号码后价格为<?= sprintf('%.0f', $data['special_price']); ?>）<br />
                    ...
                    4. 点击验证后情况（如下图）<br />
                    <img width="510" src="<?= $CURRENT_TEMPLATE_URL ?>instruction2.png" /><br />
                    5. 当您在玩途官网完成支付宝支付后，您的订单即已经成功提交了。我们将在一周内把正式的“<?= $data['product_name']; ?>“的兑换确认单（voucher）发送到您的邮箱。<br /><br />
                    注：由于您本次参加了“<?= $data['product_name']; ?>”的<?= sprintf('%.0f', $data['special_price']); ?>元限时特惠，该订单无法享受退货处理，敬请理解。
                  </td>
                </tr>
                <!-- <tr>
