...
class EmailBooking
{
    ...
    public function addBooking($order, $order_product)
    {
        ...
        if (!$isok) {
            Yii::log('addBooking: send mail to supplier failed. order_id[' . $order_product['order_id'] . ']', CLogger::LEVEL_WARNING);
        }

        //This booking just send notify to Supplier, so return not ok.
        $result['code'] = 300;//重要:不能改为200，否则会立即发货，必须等OP等到供应商确认才能发货！！！
